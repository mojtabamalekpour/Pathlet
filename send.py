...
bind_layers(Ether, SourceRoute, type=0x8000)
...
def main():
    ...
    while True:
        print
        s = str(raw_input('Type space separated pathlet IDs or port nums '
                          '(example: "3 100 200") or "q" to quit: '))
        if s == "q":
            break;
        print
        ...
        for p in s.split(" "):
            try:
                pkt = pkt / SourceRoute(bos=0, port=int(p),size=len(p))
                i = i+1
            except ValueError:
                pass
